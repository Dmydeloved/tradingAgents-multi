...
def create_multi_fundamentals_analyst(llm, toolkit):
    @log_analyst_module("multi_fundamentals")
    def multi_fundamentals_analyst_node(state):
        logger.debug(f"📊 [DEBUG] ===== 基本面分析师节点开始 =====")

        current_date = state["trade_date"]
        # ticker = state["company_of_interest"]
        # tickers = state.get("company_list") or [state.get("company_of_interest")]
        ...
        logger.info(f"当前输入的股票代码：tickers={tickers}")
        ...
        system_message = (
            f"你是一位专业的股票基本面分析师。"
            f"⚠️ 绝对强制要求：你必须调用工具获取真实数据！不允许任何假设或编造！"
            f"任务：你需要对以下多只股票进行基本面分析"
            f"{stock_list_text}"
            f"🔴 对每一只股票都必须分别调用 get_stock_fundamentals_unified 工具"
            f"参数：ticker='股票代码', start_date='{start_date}', end_date='{current_date}', curr_date='{current_date}'"
            "📊 分析要求："
            "- 基于真实数据进行深度基本面分析"
            f"- 计算并提供合理价位区间（使用对应市场货币单位）"
            "- 分析当前股价是否被低估或高估"
            "- 提供基于基本面的目标价位建议"
            "- 包含PE、PB、PEG等估值指标分析"
            "- 结合市场特点进行分析"
            ...
            "- 投资建议必须使用中文：买入、持有、卖出"
            "- 绝对不允许使用英文：buy、hold、sell"
            f"- 每只股票的货币单位需正确对应其市场（如人民币¥、港元HK$、美元$等）。"
            "🚫 严格禁止："
            "- 不允许说'我将调用工具'"
            "- 不允许假设任何数据"
            "- 不允许编造公司信息"
        ...
        system_prompt = (
            ...
            "分析目标股票清单："
            f"{stock_list_text}"
            "请确保在分析中严格区分每家公司的名称和股票代码，并分别给出分析结果。。"
        )
        ...
        # 添加详细的股票代码追踪日志
        logger.info(f"🔍 [股票代码追踪] LLM调用前，ticker参数: '{tickers}'")
        logger.info(f"🔍 [股票代码追踪] 传递给LLM的消息数量: {len(state['messages'])}")
        ...
        if GoogleToolCallHandler.is_google_model(fresh_llm):
            ...
            if tool_call_count > 0:
                ...
                try:
                    logger.debug(f"📊 [DEBUG] 强制调用 get_stock_fundamentals_unified（多股票模式）...")
                    # 安全地查找统一基本面分析工具
                    unified_tool = None
                    for tool in tools:
                        tool_name = None
                    ...
                    if unified_tool:
                        combined_data_tickers = {}  # 存储所有股票的原始数据，格式: {ticker: 数据}
                        stock_info_map = {}  # 存储股票基本信息，格式: {ticker: {company_name, currency_info}}
                        # if tickers and isinstance(tickers[0], list):
                        #     tickers = [t for sub in tickers for t in sub]
                        # 批量获取每只股票的数据和基础信息
                        for ticker in tickers:
                            # 获取当前股票的市场信息和公司名称（复用之前的market_info_map和company_name_map）
                            market_info = StockUtils.get_market_info(ticker)
                            company_name = _get_company_name_for_fundamentals(ticker,market_info)
                            currency_info = f"{market_info['currency_name']}（{market_info['currency_symbol']}）"
                            ...
                            # 调用工具获取单只股票数据
                            logger.info(f"🔍 [股票代码追踪] 强制调用统一工具，传入ticker: '{ticker}'")
                            temp_combined_data = unified_tool.invoke({
                                'ticker': ticker,
                                'start_date': start_date,
                                'end_date': current_date,
                                'curr_date': current_date
                            ...
                            # 验证数据有效性
                            if not temp_combined_data:
                                logger.warning(f"📊 [WARNING] 工具返回空数据，ticker: {ticker}")
                                combined_data_tickers[ticker] = "工具返回空数据"
                            else:
                                logger.debug(
                                    f"📊 [DEBUG] 统一工具数据获取成功，ticker: {ticker}，长度: {len(str(temp_combined_data))}字符")
                                combined_data_tickers[ticker] = temp_combined_data

                        # 生成多股票分析的prompt（按股票分组）
                        analysis_sections = []
                        for ticker in tickers:
                            info = stock_info_map[ticker]
                            data = combined_data_tickers[ticker]

                            # 单只股票的分析要求
                            section = f"""【股票代码：{ticker} - {info['company_name']}】
                基于以下真实数据，进行详细的基本面分析：
                {data}

                请提供：
                1. 公司基本信息分析（{info['company_name']}，股票代码：{ticker}）
                2. 财务状况评估
                3. 盈利能力分析
                4. 估值分析（使用{info['currency_info']}）
                5. 投资建议（买入/持有/卖出）

                要求：
                - 严格基于上述数据，不编造信息
                - 价格单位必须使用{info['currency_info']}
                - 投资建议仅限中文（买入/持有/卖出）
                """
                            analysis_sections.append(section)

                        # 组合所有股票的分析要求，形成最终prompt
                        analysis_prompt = """你是专业的多股票基本面分析师，需要对以下每只股票分别进行独立分析，按顺序生成报告：

                """ + "\n\n".join(analysis_sections) + """

                最终输出格式要求：
                1. 每只股票分析以"【股票代码：XXX - 公司名称】"开头
                2. 严格区分不同股票的分析结果，不混淆数据
                3. 所有内容使用中文，禁止英文投资建议
                4. 确保每只股票都包含完整的5项分析内容"""

                    else:
                        # 工具未找到时的提示
                        analysis_prompt = f"统一基本面分析工具不可用，无法获取以下股票数据：{', '.join(tickers)}"
                        logger.debug(f"📊 [DEBUG] 统一工具未找到")

                except Exception as e:
                    # 捕获全局异常，确保所有股票都有提示
                    error_msg = f"工具调用失败：{str(e)}"
                    analysis_prompt = f"以下股票分析失败：{', '.join(tickers)}\n错误原因：{error_msg}"
                    logger.error(f"📊 [ERROR] 统一工具调用异常: {e}", exc_info=True)
    ...
    return multi_fundamentals_analyst_node


def create_multi_fundamentals_analyst_ak(llm, toolkit):
    @log_analyst_module("multi_fundamentals")
    def multi_fundamentals_analyst_node(state):
        logger.debug(f"📊 [DEBUG] ===== 基本面分析师节点开始 =====")

from tradingagents.agents.analysts.market_analyst import create_market_analyst, create_multi_stocks_market_analyst, \
    create_multi_stocks_market_analyst_ak
...
class MultiMarketAnalyst:
    def __init__(self, llm: ChatOpenAI, toolkit: Toolkit):
        self.llm = llm
        self.toolkit = toolkit

    def run(self,state:dict):
        multi_market_agent_analyst = create_multi_stocks_market_analyst_ak(self.llm,self.toolkit)
        # 校验参数
        temp_state = parse_state(state)
        if temp_state == "":
            return ""
        ...
        try:
            ...
            return f"处理股票字典时出错: {str(e)}"
        market_report = multi_market_agent_analyst(state)

        return market_report.get("market_report","")

    def as_tool(self):
        return Tool(
            name="multi_market_analyst",
            func = lambda state:self.run(state),
            description=(
                "该工具用于对指定股票进行市场趋势与技术指标的分析，生成详细的市场分析报告。\n\n"
                "【功能说明】\n"
                ...
                "- state: Dict[str, Any]\n"
                "  包含分析上下文信息的字典，常见字段如下：\n"
                "  • trade_date (str)：分析日期，例如 '2025-10-01 14:17:05'\n"

...
@social_bp.route("/get/report",methods=["POST"])
def get_social_media_report():
    ...
    try:
        ...
        app.logger.info(f"用户开始获取social media report，trade_date:{trade_date},company_of_interest:{company_of_interest}")
        sentiment_report = get_social_media_report(state)
    except Exception as e:
        app.logger.error(f"生成social media report失败，原因：{str(e)}")
        return fail(data=str(e))
    return success(sentiment_report)
...
from webServer.services.fundamental_service import get_fundamentals_report
from webServer.utils.response import success, fail

fundamental_bp = Blueprint("fundamental",__name__)

@fundamental_bp.route("/get/report",methods=["POST"])
def get_fundamental_report():
    ...
    try:
        ...
        app.logger.info(f"用户开始获取fundamentals report，trade_date:{trade_date},company_of_interest:{company_of_interest}")
        fundamental_report = get_fundamentals_report(state)
    except Exception as e:
        app.logger.error(f"生成fundamentals report失败，原因：{str(e)}")
        return fail(data=str(e))
    return success(fundamental_report)